# ==================================================
# PATCH INSTALLER SEB - LICENSE KEY GENERATOR
# ==================================================
...
switch ($choice) {
    "1" {
        ...
        $license = New-LicenseKey -CustomerName $customerName `
            -ComputerName $computerName -DaysValid $daysValid
        ...
        Write-Host "`n[💾] License saved to licenses.csv" -ForegroundColor Gray
    }
    ...
    "2" {
        ...
        if ($licenses.Count -gt 0) {
            ...
            Write-Host "`n[💾] Saved to: bulk_licenses_$(Get-Date -Format 'yyyyMMdd').csv" -ForegroundColor Green
        }
    }
    ...
    "3" {
        ...
        if ($isValid) {
            Write-Host "`n[✅] LICENSE KEY FORMAT VALID!" -ForegroundColor Green
            Write-Host "   Computer: $computerName" -ForegroundColor White
            Write-Host "   Status: Ready for activation" -ForegroundColor White
        } else {
            Write-Host "`n[❌] INVALID LICENSE KEY FORMAT!" -ForegroundColor Red
