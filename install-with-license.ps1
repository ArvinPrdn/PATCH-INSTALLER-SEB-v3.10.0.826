...
# With License Activation System
# ==================================================
...
function Activate-License {
    ...
    try {
        ...
        New-ItemProperty -Path $licenseRegistryPath -Name $licenseValueName `
            -Value $LicenseKey -PropertyType String -Force | Out-Null
        
        # Tambahkan activation date
        New-ItemProperty -Path $licenseRegistryPath -Name "ActivationDate" `
            -Value (Get-Date -Format "yyyy-MM-dd") -PropertyType String -Force | Out-Null
        
        New-ItemProperty -Path $licenseRegistryPath -Name "ComputerName" `
            -Value $env:COMPUTERNAME -PropertyType String -Force | Out-Null
...
if ($licenseCheck.Activated) {
    ...
    Write-Host "`n[⚡] Proceeding to installation..." -ForegroundColor Cyan
} else {
    # Tampilkan activation screen
    Write-Host "`n[🔑] LICENSE ACTIVATION REQUIRED" -ForegroundColor Yellow
...
Write-Host "`n[🎉] YOUR SOFTWARE IS NOW ACTIVATED!" -ForegroundColor Magenta
...
Write-Host "`n[📋] What to do next:" -ForegroundColor Yellow
