...
function Get-SecureDownloadUrl {
    ...
    Write-Log "Requesting secure download URL from server" -Level "INFO"

    # Server endpoint for license validation and download URL
    $serverUrl = "https://api.seb-software.com/v1/download"
    ...
    try {
        ...
        if ($result.success) {
            Write-Log "Download URL obtained from server" -Level "SUCCESS"
            return $result.downloadUrl
        } else {
            throw "Server validation failed: $($result.message)"
        }

    } catch {
        Write-Log "Server request failed: $_" -Level "ERROR"

        # Fallback: Use encrypted local URL (less secure)
        Write-Log "Using fallback encrypted URL" -Level "WARNING"
        return Get-EncryptedDownloadUrl -LicenseKey $LicenseKey
    }
}
...
function Get-EncryptedDownloadUrl {
    ...
    # This is a fallback method - less secure
    Write-Log "Using encrypted fallback URL" -Level "WARNING"

    # Base64 encoded parts of GitHub URL (private repo)
    $encryptedParts = @(
        "aHR0cHM6Ly9naXRodWIuY29tL0FydmluUHJkbi9TRUItUFJJVkFURS1SRVBPL3JlbGVhc2VzL2Rvd25sb2FkL3Yz",
        "LjEwLjAuODI2L3NlYi1wcml2YXRlLmluc3RhbGxlci5leGU="
    )
    ...
    try {
        ...
        # XOR encryption with license key
        ...
        Write-Log "Encrypted fallback failed: $_" -Level "ERROR"
        throw "Unable to generate secure download URL"
    }
}
